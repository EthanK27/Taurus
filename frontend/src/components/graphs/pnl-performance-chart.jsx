...
const buildXTicks = (series, range) => {
    ...
    const stride = Math.max(
        1,
        Math.floor((series.length - 1) / (targetTickCount - 1)),
    );
    const ticks = [];
...
const buildYTicks = (series, showBenchmark) => {
    ...
    return Array.from({ length: blocks + 1 }, (_, index) =>
        Number((low + step * index).toFixed(2)),
    );
};
...
const buildVisibleWindow = ({ data, range, startDate, availableEndDate }) => {
    ...
    const endDate =
        range === "ALL" ? availableEndDate : addUTCDays(startDate, dayCount - 1);
    ...
    return data.filter(
        (entry) =>
            entry.timestamp >= startBoundary && entry.timestamp <= endBoundary,
    );
};
...
const downsample = (data, range) => {
    ...
    switch (range) {
        ...
        case "1Y":
            ...
            break;
        case "ALL":
            bucketSize = Math.ceil(data.length / 500);
            break;
        default:
            break;
    }
    ...
    for (let index = 0; index < data.length; index += bucketSize) {
        ...
        const average =
            slice.reduce((total, entry) => total + entry.pnl, 0) / slice.length;
        result.push({
...
export function PnlPerformanceChart({ runDirectory, refreshToken = "" }) {
    ...
    useEffect(() => {
        ...
        async function loadData() {
            try {
                ...
                if (!cancelled) {
                    setError("Unable to load P&L graph data.");
                }
            } finally {
                if (!cancelled) {
                    setLoading(false);
    ...
    if (loading) {
        return (
            <div className="pnl-chart pnl-chart--loading">
                <div className="pnl-chart__plot-placeholder" aria-hidden="true">
                    <div className="pnl-chart__plot-placeholder-bar pnl-chart__plot-placeholder-bar--wide" />
                    <div className="pnl-chart__plot-placeholder-bars">
                        <span />
                        <span />
                        <span />
                        <span />
    ...
    if (error) {
        return <div className="pnl-chart pnl-chart__error">{error}</div>;
    }
        ...
        <div className="pnl-chart">
            <div className="pnl-chart__calendar-panel">
                ...
                <div className="pnl-chart__calendar-summary">
                    ...
                    <div className="pnl-chart__calendar-summary-value">
                        {selectedRangeLabel}: {formatReadableDate(selectedStartDate)} -{" "}
                        {formatReadableDate(selectedEndDate)}
                    </div>
                </div>
            </div>
            ...
            <div className="pnl-chart__header">
                <div className="pnl-chart__header-row">
                    <div
                        className={`pnl-chart__pnl ${
                            positive
                                ? "pnl-chart__pnl--positive"
                                : "pnl-chart__pnl--negative"
                        }`}
                    >
                        {formatCurrency(pnl)}
                    </div>
                                    ...
                                    content={({ active, payload, label }) => {
                                        ...
                                        return (
                                            <div className="pnl-chart__tooltip">
                                                <div className="pnl-chart__tooltip-time">
                                                    {formatTimestampWithTime(label)}
                                                </div>
                                                <div>
