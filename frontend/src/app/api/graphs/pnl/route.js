...
export async function GET(request) {
    ...
    if (requestedRun?.userSeries?.length && requestedRun?.buyHoldSeries?.length) {
        return Response.json({
            userSeries: requestedRun.userSeries,
            buyHoldSeries: requestedRun.buyHoldSeries,
            spySeries: requestedRun.spySeries,
            generatedAt: requestedRun.generatedAt,
            source: "backtest-log",
            runDirectory: requestedRun.runDirectory,
        });
    }

    const latestRun = await loadLatestRunSnapshot();
    if (latestRun?.userSeries?.length && latestRun?.buyHoldSeries?.length) {
        return Response.json({
            userSeries: latestRun.userSeries,
            buyHoldSeries: latestRun.buyHoldSeries,
            spySeries: latestRun.spySeries,
